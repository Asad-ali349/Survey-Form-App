...
class Survey extends Model
{
    ...
    public function getSurveyResponseCounts(PDO $pdo)
    {
        ...
        foreach ($this->questions as $i => $question) {
            ...
            // Count of response submitted


            $sql2 = 'SELECT count(*) FROM survey_response where survey_id='.$this->survey_id;
            
            $stmt2 = $pdo->prepare($sql2);
            $stmt2->execute();
            $stmt2->setFetchMode(PDO::FETCH_NUM);
            $row2 = $stmt2->fetch();
            $this->responses_count=$row2[0];
            ...
            foreach ($question->choices as $choice) {
                ...
                $percent=($choice->answer_count*100)/$this->responses_count;
                


                $question->choice_counts[] = [$choice->choice_text, $choice->answer_count];
                $question->choice_counts_percent_age[] = [$choice->choice_text, $percent];
